...
void loadmusic(Mix_Music * fon, int n)
{
	switch (n)
	{
	case 0: fon = Mix_LoadMUS("m0.mp3"); break;
	case 1: fon = Mix_LoadMUS("m1.mp3"); break;
	case 2: fon = Mix_LoadMUS("m2.mp3"); break;
	}
	Mix_PlayMusic(fon, -1);
}
...
void init_text(SDL_Renderer *renderer, SDL_Texture* text_button[], int i_menu, TTF_Font* font, int theme)
{
	...
	case 1:
	{
		for (int j = 0; j < 3; j++)
		{
			...
			{ 
				char text[] = "DRAGONS";
				text_button[0] = get_text_texture_text(renderer, font, text, theme);
			}
			if (j == 1) 
			{
				char text[] = "BTS";
				text_button[1] = get_text_texture_text(renderer, font, text, theme);
			}
			if (j == 2) 
			{ 
				char text[] = "CLASSIC";
				text_button[2] = get_text_texture_text(renderer, font, text, theme);
			}
		}
	} break;
	case 2: // main menu
...
void init_rect(SDL_Rect button[], int sign, int lv, int j)
{
	...
	if (sign == 3) // рекорды. врем€
	{
		button[j] = { 200, 200 + (j - 5) * 51, lv * 30, 50 };
	}
	if (sign == 4) // рекорды. им€
	{
...
int time_win(int time, int winner_time[], int lv, int emd, char winner_name[5][12], char t_name[])
{
	...
	for (int i = 0; i < 5; i++) // провер€ем новое врем€
	{ 
		for (int j = 0; j < 5; j++)
		{
...
{
	if ((x > buttom.x) && (x < (buttom.x + buttom.w)) && (y > buttom.y) && (y < (buttom.y + buttom.h))) return 1;
	else return 0;
}
