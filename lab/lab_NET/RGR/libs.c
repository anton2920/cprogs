...
int direct_forwarding(ssh_session session, const char *remotehost, int remoteport, int localport) {
    ...
    auto char buffer[0xFFFF];
    sprintf(buffer, "GET / HTTP/1.1\nHost: %s\n\n", remotehost);

    auto nbytes = strlen(buffer);
    auto nwritten = ssh_channel_write(forwarding_channel, buffer, nbytes);
    if (nbytes != nwritten) {
        ssh_channel_free(forwarding_channel);
        return SSH_ERROR;
    }

    while ((nbytes = ssh_channel_read(forwarding_channel, buffer, sizeof(buffer), 0))) {
